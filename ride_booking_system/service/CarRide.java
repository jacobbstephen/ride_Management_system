...
public class CarRide extends RideServiceAbstract {
	...
	public Ride bookRide(User user, String pickup, String drop) throws DriverNotFoundException {
		...
		for(int i = 0; i < driverCount; i++) {
			if(drivers[i].isAvailable() && drivers[i].getVechileType().equalsIgnoreCase("car")) {
				assignedDriver = drivers[i];
				assignedDriver.setAvailable(false);
				break;
			}
		}
		if(assignedDriver == null) throw new DriverNotFoundException("All Drivers are busy. Couldn't assign driver for your rideUser: " + user.getName());
